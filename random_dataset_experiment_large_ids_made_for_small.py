...
def run_experiment(config_path):
    ...
    print(f"reading config path: {config_path}")
    config.read(config_path)

    # with open ("/home/mandy/project/facial_feature_impact_comparison/configs/asian_new_random_config_vggface_trained_for_cf_vi.cfg") as f:   ## here you get an exception when file not exists.
    #     s = f.read()
        
    # cfg = configparser.RawConfigParser()
    # cfg.read_string(s)  

    print(f"finish reading config path: {config}")
    ...
    for id in num_ids:
        # num_iterations changes according to id num (smalls-30, 100 - only 10, large- 1)
        if int(id) >100:
            num_iterations = 1
            print(f'id > 100, changed num_iterations to: {num_iterations}')
        experiment_name = config['GENERAL']['base_experiment_name'] # small ids: #+'_'+ str(id) + '_ids'
        ...
        print('experiment name_rd:', experiment_name)
        ...
        for pic in num_pics:
            ...
            for i in range(num_iterations):
                ...
                # # for large ids:
                # imgs_name = 'ids_' + str(pic) + '_img'
                # orig_img_name = 'ids_' + config['GENERAL']['base_experiment_name'].split('_')[2] + '_img'
                # print('orig_img_name:', orig_img_name)
                # ids_name = str(id) + '_ids_'
                # orig_id_name = config['GENERAL']['base_experiment_name'].split('_')[0] + '_ids_'
                # print('orig_id_name:', orig_id_name)
                # new_base_exp_name = config['GENERAL']['base_experiment_name'].replace(orig_id_name ,ids_name )
                # new_base_exp_name = new_base_exp_name.replace(orig_img_name ,imgs_name )

                # print('new_base_exp_name: ', new_base_exp_name)
                # # config['GENERAL']['base_experiment_name'] = new_base_exp_name
                # # config['GENERAL']['root_dir'] =new_base_exp_name
                # # config['GENERAL']['experiment_name'] =new_base_exp_name
                # experiment_name = new_base_exp_name
                print('new exp name: ', experiment_name)
                ...
                with mlflow.start_run(run_name=run_name):
                    ...
                    config['GENERAL']['experiment_name'] = experiment_name + '_' + str(id) + '_' + 'ids' + '_' + str(id) + '_' + str(pic) + '_' + str(i)
                    ...
                    # #for VAL testing, added in order to load pretrained:
                    # model = ModelInitializer(json.loads(config['MODELLING']['feature_parallelized_architectures'])).get_model(config['MODELLING']['architecture'], False)
