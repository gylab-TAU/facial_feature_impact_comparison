...
def get_lfw_test(config, image_loader):
    ...
    with open(labeled_pairs_path, 'r') as f:
        for line in f:
            labeled_pairs_list.append(line.split(' '))
    ...
    return lfw_test.LFWTester(labeled_pairs_list, reps_cache_path, image_loader, RepAccuracyTester(threshold_matcher),  comparison_calc)